...
{  
    ...
    { 
        ...
        static public string AdjustDPI(int adjust) {
            User_32.SystemParametersInfo(User_32.SPI_SETLOGICALDPIOVERRIDE, adjust, IntPtr.Zero, 1);
            return "";
        }
        static public string ListModes(int deviceIDIn) {
            //Basic Error Check 
            uint deviceID = 0; 
            if (deviceIDIn < 0){ 
                deviceID = 0; 
            ...
            for (int i = 0; true; i++) {
                if (0 == User_32.EnumDisplaySettingsEx ( d.DeviceName, i, ref dm, 0)) {
                    break;
                }
                Console.WriteLine("{5}: {0} x {1} {2}Bits {3}Hz {4}", dm.dmPelsWidth, dm.dmPelsHeight, dm.dmBitsPerPel, dm.dmDisplayFrequency, dm.dmDisplayFlags, i);  
            }
            
            return "";
        }
        static public string SetMode(int deviceIDIn, int mode_index) 
        {  
            //Basic Error Check 
            uint deviceID = 0; 
            if (deviceIDIn < 0){ 
            ...
            if (0 != User_32.EnumDisplaySettingsEx ( d.DeviceName, mode_index, ref dm, 0))  
            {  
